...
% However, we couldn't reproduce the results of the paper.

% Even without the trigger present in the recording, we could not observe a random distribution in the entropy. We followed the paper step by step and even a GitHub repository which is the source code of the Strip Vita paper. We could reproduce the same graphs instead of superimposing using the training dataset; we used random data in the superimposing step. Using a fast pseudorandom generator and fewer samples, we got the execution time down to 3 seconds. We also got the following distributions:
% \begin{figure}[!hbt]
